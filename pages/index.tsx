...
const Test: FC<CssProps> = (props) => {
  ...
  console.log(test);
  return <>
    <div className={cn(test.styleArray.map(([className])=>className))}>{props.children}</div>
    {
      test.styleArray.map(([className, style]) => <_JSXStyle key={className} id={className}>{style}</_JSXStyle>)
    }
  </>;
};
